...
set_modem_config()
{
    ...
    [ -n "$state" ] && [ "$state" != "disabled" ] && temp_cfg=$cfg
}
...
{
    unset temp_cfg
    first_avalible_config=$(get_first_avalible_config)
    config_load modem_sim
    config_get detect_interval global detect_interval 5
    config_get judge_time global judge_time 5
    config_get sim_auto_switch global sim_auto_switch 0
    config_get modem_config global wwan_ifname $first_avalible_config
    config_get ping_dest global ping_dest
}
...
main_monitor() {
    ...
    while true; do
        ...
        if [ $? -eq 1 ]; then
            ...
            update_config
            logger -t modem_sim "precheck failed,update config. detect_interval:$detect_interval judge_time:$judge_time sim_auto_switch:$sim_auto_switch modem_config:$modem_config ping_dest:$ping_dest"
            continue
        fi
