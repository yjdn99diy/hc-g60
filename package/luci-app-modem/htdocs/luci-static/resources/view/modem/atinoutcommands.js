...
return view.extend({
	...
	handleGo: function(ev) {
		...
		if ( !port )
			...
			return this.handleCommand('/usr/bin/luci-app-atinout', [ atcmd,  port ]);
			}
		}
	...
	load: function() {
		return Promise.all([
			L.resolveDefault(fs.read_direct('/etc/modem/atinoutatcommands.user'), null),
			uci.load('atinout')
		]);
	},
	...
	render: function (loadResults) {
		...
		return E('div', { 'class': 'cbi-map', 'id': 'map' }, [
				E('h2', {}, [ _('AT Commands') ]),
				E('div', { 'class': 'cbi-map-descr'}, _('User interface for sending AT commands using atinout utility.')),
				E('hr'),
